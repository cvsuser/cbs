...
void cbs_httpd_service(void *arg);
void cbs_parse_request(int sockid);
int cbs_clear_buffer();
void cbs_recver_service(void *arg);
...
static int port = 35536;
static int first_send = 0;
static FILE* dump_file = NULL;
static int g_getRt10up = 0;
static int g_read_pipe = 0;
static int g_write_pipe = 0;
pthread_mutex_t mutex;

#define LISTENFD 5
static int listener_fd[LISTENFD] = {-1};

static int cbs_hotplug_start(void)
{
    int fd = 0;
...
static int cbs_hotplug_process(int fd)
{
    char buf[4096] = {0};
...
static int cbs_message_broadcast(char *s){
...
void cbs_hotplug_service(void *arg){

    ALOGD("%s %d. pthread start.\n",__FUNCTION__,__LINE__);
    g_getRt10up = ! cbs_hotplug_check();
    ALOGD("%s %d,g_getRt10up = %d\n",__FUNCTION__,__LINE__,g_getRt10up);
    int hotplug_fd = cbs_hotplug_start();
    int old = g_getRt10up;
    while (1) {
        int ret = cbs_hotplug_process(hotplug_fd);
        if(ret >= 0 ){
            g_getRt10up = ret;
        }
        ALOGD("%s %d,g_getRt10up = %d\n",__FUNCTION__,__LINE__,g_getRt10up);
        if(g_getRt10up != old){
            old = g_getRt10up;
            cbs_message_broadcast("hotplug:g_getRt10up change!");
        }
    }
    close(hotplug_fd);
    ALOGD("<%s:%d>, exit[%d]%s\n",__FUNCTION__,__LINE__, errno,strerror(errno)  );
...
void cbs_httpd_service(void *arg)
{
    ALOGD("%s %d. pthread start.\n",__FUNCTION__,__LINE__);
    int http_fd = -1;
    while((http_fd = cbs_http_start()) < 0){
        sleep(2);
    }
    ...
    while(1)    {//TODO
        ...
        if(connfd == -1) {
            ...
            if(g_getRt10up == 1){
                ...
                cbs_parse_request(connfd);
                fcntl(connfd, F_SETFL, O_NONBLOCK);
                g_clientSocketFd = connfd;
                cbs_message_broadcast("httpserver:GET new socket!");
            }else{
                close(connfd);
            }
        }
...
static void cbs_parse_request(int fd)
{
    char buff[4096] = {0};
...
void cbs_sender_service(void *arg){
    ...
    while(1){
        ...
        FD_SET(pipe_fd[0], &rdfds);
        int max_fd = max(pipe_fd[0],g_clientSocketFd);
        ret = select (max_fd + 1, &rdfds, &wdfds, NULL, &tv);

        if(ret < 0){
            ALOGD("ret = %d,socket=[%d],[%d]%s\n",ret,g_clientSocketFd,errno,strerror(errno)  );
            if(errno == 9 && g_clientSocketFd != -1) {
                close(g_clientSocketFd);
                g_clientSocketFd = -1;
            }
        } else if (ret == 0){
        ...
        }/*else if(g_clientSocketFd != -1 && FD_ISSET(g_clientSocketFd, &rdfds)){
            char buf[512] = {0};
            ret = read(g_clientSocketFd,buf,512);
            if(ret != 0)
                ALOGD("%s %d,rdfds,buf=%s,ret = %d\n",__FUNCTION__,__LINE__,buf,ret);
        } */
        else if(g_clientSocketFd != -1 && FD_ISSET(g_clientSocketFd, &wdfds)){
...
int main(int argc, char ** argv)
{
    ...
    int ret = pthread_create(&pid1,NULL,&cbs_httpd_service,NULL);
    ret = pthread_create(&pid2,NULL,&cbs_sender_service,NULL);
    ret = pthread_create(&pid3,NULL,&cbs_recver_service,NULL);
    ret = pthread_create(&pid4,NULL,&cbs_hotplug_service,NULL);
