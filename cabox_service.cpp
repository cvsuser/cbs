...
#define LISTENFD 5
int listener_fd[LISTENFD] = {0};

int send_msg(char *s){
    for(int i = 0;i < LISTENFD; i++){

        if(listener_fd[i] > 0){
            int ret = write(listener_fd[i],s,strlen(s));
			ALOGD("ret = %d,listener_fd[i] =%d,[%d]%s",ret,listener_fd[i], errno,strerror(errno)  );
		}
    }
    return 0;
}

#define SERVICEFILE  "/var/tmp/media/server_socket"
int start_msg_service()  
{
  ...
  ALOGD("[%d]socket=[%d],[%d]%s\n",__LINE__,server_sockfd,errno,strerror(errno)  );
  struct sockaddr_un server_addr;  
  server_addr.sun_family = AF_UNIX;  
  strcpy(server_addr.sun_path, SERVICEFILE);  
...
void local_server(void *arg)
{
	...
	while(1){
		...
		FD_SET(msg_fd, &rdfds);
		FD_SET(http_fd, &rdfds);
		FD_SET(hotplug_fd, &rdfds);
		
		int ret = select (255, &rdfds, NULL, NULL, NULL);
		if(ret < 0){
			ALOGD("[%d]ret = %d,socket=[%d],[%d]%s\n",__LINE__,ret,g_clientSocketFd,errno,strerror(errno)  );
		} else if (ret == 0){
			ALOGD("%s %d,time out\n",__FUNCTION__,__LINE__);
		} else if(FD_ISSET(http_fd, &rdfds)){
			int clieLen = 0;
			...
			if(connfd == -1) {
				...
				if(g_getRt10up == 1){
					...
					ret = setsockopt(connfd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse0,sizeof(reuse0));
					...
					char bus[512]={0};
					sprintf(bus,"%d=httpserver:GET new socket!",connfd);
					send_msg(bus);
				}else{
					close(connfd);
				}
			}
		} else if(FD_ISSET(msg_fd, &rdfds)){
			int fd;  
			struct sockaddr_un client_addr;  
			socklen_t len = sizeof(client_addr); 
			/* accept a connection */  
			fd = accept(msg_fd, (struct sockaddr *)&client_addr, &len);
			if(fd == -1) {
				ALOGD("%s %d,connfd == -1\n",__FUNCTION__,__LINE__);
				usleep(2000);
			} else {
				for(int i = 0;i < LISTENFD; i++){
					ALOGD("\t<%s>%d,pipe = [%d:%d]",__FUNCTION__,__LINE__,listener_fd[i],fd);
					if(listener_fd[i] <= 0){
						listener_fd[i] = fd;
						break;
					}
		...
		}else if(FD_ISSET(hotplug_fd, &rdfds)){
			int ret = process_hotplug(hotplug_fd);
			if(ret >= 0 ){
				g_getRt10up = ret;
			}
...
{  
  ...
  int sockfd = socket(AF_UNIX, SOCK_STREAM, 0);  
    ALOGD("[%d]socket=[%d],[%d]%s\n",__LINE__,sockfd,errno,strerror(errno)  );
  struct sockaddr_un address;  
  address.sun_family = AF_UNIX;  
  strcpy(address.sun_path, SERVICEFILE);  
...
while(1){
    while(g_clientSocketFd == -1 || g_getRt10up == 0){
        ...
        ret = select (msg_fd + 1, &rdfds, NULL, NULL, NULL);
        if(ret < 0){
            ALOGD("[%d]ret = %d,socket=[%d],[%d]%s\n",__LINE__,ret,g_clientSocketFd,errno,strerror(errno)  );
        } else if (ret == 0){
            ALOGD("%s %d,time out\n",__FUNCTION__,__LINE__);
        } else if(FD_ISSET(msg_fd, &rdfds)){
            ret = read(msg_fd,data_g,512);
            ALOGD("\n**************--->%s,getmsg=%s.[%d]\n\n",__FUNCTION__,data_g,ret);
        }
    }
    if(g_getRt10up == 1){
    ...
    while(g_clientSocketFd != -1 && g_getRt10up == 1) {
        gettimeofday (&f2, NULL);
        int ll = (f2.tv_sec-f1.tv_sec)*1000+(f2.tv_usec-f1.tv_usec)/1000;
        if(ll > 3000){
            if(sum != 0)
                total_time += ll;
            ALOGD("recv = %4d ms, size = %4d KB, wp =%lld,rd=%lld,total time = %f Secs\n", ll,sum/1024,writeOffset,readOffset,total_time/1000);
            gettimeofday (&f1, NULL);
            sum = 0;
        }
    ...
    if(av1_start!=NULL && fifo_ptr!=NULL){
    ...
    while(1){
        ...
        while(g_clientSocketFd == -1){
            ...
            if(ret < 0){
                ...
                char buf[512] = {0};
                ret = read(msg_fd,buf,512);
                ALOGD("\n**************--->%s,getmsg=%s[fd = %d]\n\n",__FUNCTION__,buf,g_clientSocketFd);
            }
        }
        ...
        ret = select (g_clientSocketFd+1, NULL, &wdfds, NULL, &tv);
		//ALOGD("[%d]ret = %d,socket=[%d],[%d]%s,wp=%lld,rd=%lld\n",__LINE__,ret,g_clientSocketFd,errno,strerror(errno), writeOffset,readOffset );
        if(ret < 0){
            ALOGD("[%d]ret = %d,socket=[%d],[%d]%s,wp=%lld,rd=%lld\n",__LINE__,ret,g_clientSocketFd,errno,strerror(errno), writeOffset,readOffset );
            if(errno == 9) {
            ...
            if(ret > 0)
                ALOGD("%s %d,rdfds,buf=%s,ret = %d\n",__FUNCTION__,__LINE__,buf,ret);
		            ...
		            char buf[512] = {0};
            ret = read(g_clientSocketFd,buf,512);
		ALOGD("[%d]ret = %d,socket=[%d],[%d]%s,wp=%lld,rd=%lld,ret=%d\n",__LINE__,ret,g_clientSocketFd,errno,strerror(errno), writeOffset,readOffset,ret );
